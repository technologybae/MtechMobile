...
where " . dbObject . "EMP.WebCode = '" . $_SESSION['code'] . "' ");
                                ...
                                <div class="mb-3 overflow-hidden">
									...
									<button type="button" class="btn btn-info btn-actions float-start back text-white arBtn" onclick="loadPage('salesreturn_voucher_type.php');"><?= getArabicTitle('Back') ?> <i class="fa fa-arrow-right icon-font"></i></button>

									<button type="button" class="btn btn-danger btn-actions float-end submit-next enBtn" onClick="loadPage('list_salesreturn_voucher.php')">Clear</button>
									<button type="button" class="btn btn-danger btn-actions float-end submit-next arBtn" onClick="loadPage('list_salesreturn_voucher.php')"><?= getArabicTitle('Clear') ?></button>
...
<div>
<select class="form-select" name="LanguageId" id="LanguageId" aria-label="sales-men" >
...
where " . dbObject . "EMP.WebCode = '" . $_SESSION['code'] . "' ");
                                ...
                                //$sBid = !empty($_GET['sBid']) ? $_GET['sBid']: 1;
                                //$LanguageId = !empty($_GET['LanguageId']) ? $_GET['LanguageId']: 1;
                                $pages = new Paginator;
                                //$mainStoreProcedure = "EXEC  " . dbObject . "SPSalesSelectWeb   @SrchBy='".$SrchBy."',@Billno='".$Billno."',@Bid='".$Bid."',@sBid='".$sBid."',@LanguageId =".$LanguageId;



                                ///// Get Total Count/////
                                //echo "Select count(Billno) as totalrec from dataout   $condition";
                                $rf = Run("Select count(Billno) as totalrec from dataoutreturn   $condition");
                                ...
                                $OrderBy = "Order by dataoutreturn.Billno DESC";

                                $qpt = "Select * from dataoutreturn $condition  $OrderBy " . $pages->limit . "";
                                $result = Run($qpt);
                                ?>
                                <div class="mb-3">
                                    <!-- Listing -->
                                        ...
                                        if ($pages->items_total > 0) { ?>
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th><span class="en">Type</span><span class="ar"> <?= getArabicTitle('Type') ?></span></th>
                                                ...
                                                ?>
                                                    <tr>
                                                        <td style="float: left"><?= $single->BillNo ?>
                                                            <a href="#" onclick="openPopup('<?= $single->BillNo ?>','<?= $single->Bid ?>','<?= $email ?>')" style="width: fit-content;float: left">
                                                                <span class=""><i class="fa fa-envelope-o" aria-hidden="true"></i></span>
                                                            </a>

                                                            <a href="javascript:openPrintPopup('<?= $single->BillNo ?>','<?= $single->Bid ?>');" style="width: fit-content; float: left">
                                                        ...
                                                        <td><?= $typ ?></td>
                                                        <td><?= AmountValue($single->vatPTotal) ?></td>
            ...
            <div class="modal-content">
                ...
                <form action="javascript:sendemailform()">
                    <div class="modal-body">
                        ...
                        <select class="form-control " id="email_lang" name="email_lang">
                            <option value="1">English</option>
                            <option value="2">Arabic</option>
                        </select>
                    </div>
