...
class Stripe {
	 ...
	 */
	public function charge_customer( $amount, $customer_id, $desc, $metadata = array() ) {
		$params = array(
			'amount' => $amount,
			'currency' => 'usd',
	 ...
	 * @param  string        A free form reference for the charge
	 */
	public function charge_card( $amount, $card, $desc, $metadata = array() ) {
		$params = array(
			'amount' => $amount,
			'currency' => 'usd',
			'card' => $card,
	 ...
	 */
	public function customer_create( $card, $email, $metadata, $desc = NULL, $plan = NULL ) {
		$params = array(
			'card' => $card,
			'email' => $email,
			'metadata' => $metadata
	...
	private function _send_request( $url_segs, $params = array(), $http_method = 'get' ) {
		if( $this->_conf['stripe_test_mode'] )
			...
			$key = getSiteSettings('stripe_key_live_secret');
