...
class ApiClient
{
	...
	const SITE_NAME = 'dumpskey.com';
	const API_KEY = 'PkQQnLKKA0C3vaJBnuFJTXIYGpX8+G9Dj9e0ZMxODBk=';
	...
	private function sendRequestNew($method, $url, $data = NULL)
	{
		$this->clearError();
	...
	/**
	 * Extends of reduces the license entitlement for the given key. If a negative number of users is specified,
	...
	public function revokeLicense(array $keys)
	{
		...
		return $this->sendRequestNew("POST", $uri, $keys);
	}
	 ...
	 * @return void
	 */
	public function downloadProduct($productId, $type, $user_email)
	{
		...
		if((bool)$this->hasError())
		{
			...
			addtoActivityLog($productId." ".$product_type." Download Failed",$user_email,'','','',0,'');
			return;
		}
		else
		{
			...
			addtoActivityLog($productId." ".$product_type." Downloaded",$user_email,'','','',0,'');
			ob_clean();
			header('Location: ' . $url);
			die();
		}			
