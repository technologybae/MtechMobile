    ...
    $.post(
      ...
      function (data) {
        ...
        var GenericHtml =
          ...
          "</td><td>" +
          Sprice +
          '</td><td><a href="#" onclick="deleteRow(' +
          row_count +
...
function fetchProductDetails(vvl, Uid) {
  if (vvl != "") {
    ...
    $.post(
      "includes/sales/fetchProductDetails.php",
      {
        code: vvl,
        Bid: Bid,
        Uid: Uid,
...
function fetchProductUnits(vvl) {
  if (vvl != "") {
    ...
    $.post(
      "includes/sales/fetchProductUnits.php",
      {
        code: vvl,
        Bid: Bid,
      },
...
function LoadUnitPrice(Uid, vvl) {
  if (vvl != "" && Uid != "") {
    ...
    document.getElementById("fetchProductDetails").innerHTML =
      "<img width='80px' src='loader/wheel.gif'/>";
    $.post(
      "includes/sales/LoadUnitPrice.php",
      {
        code: vvl,
        Bid: Bid,
        Uid: Uid,
...
function fetchProductDetailsFromCode(vvl, Uid) {
  if (vvl != "") {
    ...
    $.post(
      "includes/sales/fetchProductDetailsFromCode.php",
      {
        code: vvl,
        Bid: Bid,
        Uid: Uid,
...
function getProductList(vvl) {
  if (vvl != "") {
    ...
    $.post(
      "includes/sales/getProductList.php",
      {
        code: vvl,
      },
      function (data) {
...
function Pricecalculations(vvl, tp) {
  ...
  if (vatPer != "0") {
    $.post(
      "includes/sales/Pricecalculations.php",
      {
        vvl: vvl,
        tp: tp,
        vatPer: vatPer,
...
function PricecalculationsRow(vvl, tp, row_count) {
  ...
  if (vatPer != "0") {
    $.post(
      "includes/sales/PricecalculationsRow.php",
      {
        vvl: vvl,
        tp: tp,
        vatPer: vatPer,
...
function qtyPriceTotal(row_count) {
  ...
  var CostPrice = $("#CostPrice" + row_count).val();

  var costTotal = ((parseFloat(CostPrice) * parseFloat(vvl))).toFixed(2);
  $("#costTotal" + row_count).val(costTotal);
...
function saveFinalStep(SBBillno, Bid) {
  $.post(
    "includes/sales/saveFinalStep.php",
    {
      SBBillno: SBBillno,
      Bid: Bid,
    },
...
function printInvoice() {
  ...
  document.getElementById("printInvoice").innerHTML =
    "<img width='80px' src='loader/wheel.gif'/>";
  $.post(
    "includes/sales/printInvoice.php",
    {
      Bid: Bid,
      Billno: Billno,
...
function Print_details() {
  ...
  window.open(
    "includes/sales/print.php?Billno=" +
      bill_id +
      "&Bid=" +
      b_id +
      "&LanguageId=" +
